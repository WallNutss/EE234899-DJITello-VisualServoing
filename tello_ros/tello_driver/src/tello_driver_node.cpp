...
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_LOAD_PARAMETER((*this), cxt, n, t, d)
  ...
  void TelloDriverNode::cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    // Personal Note
    // idk why the reason the maker multiply especially linear.y and angular.z to minus,but
    // with the result of that we can get the axis frame xyz where +z to upward and rotation anti-clockwise
    // where +x forward and +y to the left
